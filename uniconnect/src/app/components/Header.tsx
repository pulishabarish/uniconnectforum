...
export const Header: React.FC = () => {
  ...
  const loadClassCounter = useCallback(() => {
    ...
    fetch(`http://localhost:8000/api/class-join-requests/?${params.toString()}`)
      .then(res => res.json())
      .then(data => {
  ...
  const loadFacultyRequests = () => {
    fetch('http://localhost:8000/api/faculty_requests/')
      .then(res => res.json())
  ...
  const loadPendingTopicRequestsCount = () => {
    fetch('http://localhost:8000/api/pending-topic-requests/')
      .then(res => res.json())
  ...
  const approveFaculty = async (userId: number) => {
    try {
      const res = await fetch('http://localhost:8000/api/approve_faculty/', {
        method: 'POST',
