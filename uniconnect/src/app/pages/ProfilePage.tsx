...
export const ProfilePage: React.FC = () => {
    ...
    const loadProfileActivity = async () => {
      try {
        const [topicsRes, repliesRes, notificationsRes, engagementRes, timelineRes] = await Promise.all([
          fetch(`http://127.0.0.1:8000/api/user-topics/${currentUser.id}/`),
          fetch(`http://127.0.0.1:8000/api/user-replies/${currentUser.id}/`),
          fetch(`http://127.0.0.1:8000/api/notifications/${currentUser.id}/`),
          fetch(`http://127.0.0.1:8000/api/user-engagement-summary/${currentUser.id}/`),
          fetch(`http://127.0.0.1:8000/api/topic-request-timelines/${currentUser.id}/`),
        ]);
  ...
  const persistProfile = async (nextUser: User) => {
    const res = await fetch('http://127.0.0.1:8000/api/update-user-profile/', {
      method: 'POST',
